...
class Note extends FlxSprite
{
	...
	public var luckyAnim:Bool = false;
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?isAlt:Bool = false, ?bet:Float = 0, ?noteSkin:String = 'normal', isluckyAnim:Bool = false)
	{
		super();
		...
		if (inCharter)
		{
			frames = Paths.getSparrowAtlas('notetypes/bfnotes');
		...
		else
		{
			...
			{
				case 'pixel':
					if (PlayState.SONG.stage == 'jevilbg')
						{
						   loadGraphic(Paths.image('weeb/pixelUI/jevil-pixels', 'shared'), true, 17, 17);
						}
						else
							{
						...
						if (isSustainNote)
							if (PlayState.SONG.stage == 'jevilbg')
								{
									loadGraphic(Paths.image('weeb/pixelUI/jevilEnds', 'shared'), true, 7, 6);
								}
								else
									{
						...
						updateHitbox();
				default:
					frames = Paths.getSparrowAtlas('notetypes/' + noteSkin, 'shared');
					...
					animation.addByPrefix('luckyp', 'lNote Left');
					animation.addByPrefix('luckyg', 'lNote UP');
					animation.addByPrefix('luckyr', 'lNote Right');
					animation.addByPrefix('luckyb', 'lNote Down');
		...
		if (isluckyAnim)
			{
				...
				{
					case 0:
						animation.play('luckyp');
					case 1:
						animation.play('luckyb');
					case 2:
						animation.play('luckyg');
					case 3:
						animation.play('luckyr');
				}
			}
			else
				{
