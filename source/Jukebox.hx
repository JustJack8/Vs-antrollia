...
class Jukebox extends MusicBeatState
{
	public static var songs:Array<SongMetadatab> = [];
	...
	override function create()
	{
		...
		for (i in 0...initSonglist.length)
		{
			...
			var meta = new SongMetadatab(data[0], Std.parseInt(data[2]), data[1]);
			...
			if (FileSystem.isDirectory("assets/sm/" + i))
			{
				...
				for (file in FileSystem.readDirectory("assets/sm/" + i))
				{
					...
					if (file.endsWith(".sm") && !FileSystem.exists("assets/sm/" + i + "/converted.json"))
					{
						...
						var meta = new SongMetadatab(file.header.TITLE, 0, "sm",file,"assets/sm/" + i);
						songs.push(meta);
		...
		//trace("\n" + diffList);
		 ...
		 DiscordClient.changePresence("listening to cool beats", null);
		 #end
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('Vs_antrollia_menu_1'));
		 ...
		 */
		 ...
		 var checkthing = new FlxUICheckBox(20, 20, null, null, "", 100);
		 checkthing.checked = false;
		 checkthing.callback = function()
		 {
			 playvocals = checkthing.checked;
		 };
		 add(checkthing);
		 checkthing.scale.set(2,2);

		 aaa = new FlxText(50, 13, 0, "Vocals", 24);
		 aaa.font = scoreText.font;
		 add(aaa);
	...
	{
		songs.push(new SongMetadatab(songName, weekNum, songCharacter));
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted)
		{
			...
			vocals = new FlxSound().loadEmbedded(Paths.juke(songs[curSelected].songName));
